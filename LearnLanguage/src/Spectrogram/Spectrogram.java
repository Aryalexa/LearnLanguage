...
public class Spectrogram{
	 ...
	 * @param wave
	 */
	public Spectrogram(double[] data,int fs) {
		//this.wave=wave;
		this.data = data;
		this.fs = fs;
	 ...
	 * Constructor
	 * 
	 * @param data double array with time data
	 * @param fs frequency sample rate
	 ...
	 */
	public Spectrogram(double[] data,int fs, int fftSampleSize, int overlapFactor) {
		//this.wave=wave;
		this.data = data;
		this.fs = fs;
		
		if (Integer.bitCount(fftSampleSize)==1){ // power of 2?
	...
	private void buildSpectrogram(){
		...
		double[] amplitudes = data;//wave.getSampleAmplitudes();
		...
		int seconds = numSamples / fs; // s = m / (m/s)
		framesPerSecond = (int)(numFrames/seconds);	// dato sin importancia
        if (DEBUG) {
        	System.out.println("number samples:"+numSamples+
        		", fftSampleSize:"+fftSampleSize+
        		", numFrames:"+numFrames+
        		", real total:"+fftSampleSize*numFrames);
        }
		...
		for (int i=0; i<numFrames; i++){			
			absoluteSpectrogram[i] = fft.getMagnitudes2(signals[i]);// 1 o 2? 2
		}
			
		if (absoluteSpectrogram.length>0){
				
			numFrequencyUnit=absoluteSpectrogram[0].length;
			unitFrequency=(double)fs/2/numFrequencyUnit;	// frequency could be caught within the half of nSamples according to Nyquist theory
	 ...
	 * 
	 * @return	logarithm normalized spectrogram ([numFrames][numFrequencyUnit])
	 */
	 ...
	 * Get spectrogram: spectrogram[time][frequency]=intensity
	 * 
	 * @return	absolute spectrogram ([numFrames][numFrequencyUnit])
	 */
