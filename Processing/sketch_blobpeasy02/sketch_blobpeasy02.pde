...
void setup() {
  ...
  String inputName = "kor3colores2";//colores;text.txt
  String[] lines = loadStrings(inputName);
  int size = lines.length;
  int w = 256; // WIDTH = CHUNK_SIZE
  int h = size/w;
...
void printPixelsValue(){
  ...
  System.out.println("img w:"+img.width+"  blobD0 w:"+theBlobDetection[0].imgWidth+"  blobD1 w:"+theBlobDetection[1].imgWidth);
  System.out.println("img h:"+img.height+"  blobD0 h:"+theBlobDetection[0].imgHeight+"  blobD1 h:"+theBlobDetection[1].imgHeight);

  
  System.out.println("pixels width: 0<"  +wi+"<"+wf+  "<"+img.width);
  System.out.println("pixels height: 0<"     +h+      "<"+img.height);
  for (int w=wi; w<wf; w++){
    //System.out.println("BD-"+theBlobDetection[0].pixels[h*theBlobDetection[0].imgWidth + w]); //they are the same
    System.out.println(                "IM-"+img.pixels[h*img.width + w]);
  }
}
